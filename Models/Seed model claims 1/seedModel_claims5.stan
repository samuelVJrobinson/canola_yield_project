...
parameters {
	//Claim 5: pollen~hbeeDist
	real slopeHbeeDistPol;
...
transformed parameters {			
	...
	for(i in 1:Nplot){
		...
		pollenMu_plot[i] = intPol + intPol_field[plotIndex[i]] + intPol_plot[i] + 
			...
			slopeHbeeDistPol*logHbeeDist_all[i]; //Claim
	}
...
model {
	...
	sigmaPolPlot ~ gamma(12,20); //Sigma for random plot	
