...
parameters { 
	//Claim: flower survival ~ hbee visitation
	real slopeHbeeVisSurv; 
...
transformed parameters {			
	...
	for(i in 1:Nplot){
		...
		flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[i]] + intFlwSurv_plot[i] + 						
			...
			slopeHbeeVisSurv*logHbeeVis_all[i]; //Effect of (log) honeybee visits
	}	
