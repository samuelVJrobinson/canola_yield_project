...
parameters { 
	//Claim: FlwCount ~ lbee stocking
	real slopeLbeeStockingFlwCount; 
...
transformed parameters {			
	...
	for(i in 1:Nplot){			
		...
		flwCountPlot[i] = intFlwCount + intFlwCount_field[plotIndex[i]]+ //+ intFlwCount_plot[i];
			slopeLbeeStockingFlwCount*lbeeStocking[i]; //Lbee stocking effect
	}	
...
model {	
	...
	// Priors	
	//Claim
	slopeLbeeStockingFlwCount ~ normal(0,1); 
