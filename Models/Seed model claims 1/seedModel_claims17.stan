...
parameters {
	//Claim 17: seedWeight ~ lbee
	real slopeLbeeSeedWeight; //Direct effect of leafcutter visitation
...
transformed parameters {			
	...
	for(i in 1:Nplant){
		...
		seedWeightPlantMu[i] = intSeedWeight + intSeedWeight_field[plotIndex[plantIndex[i]]] + intSeedWeight_plot[plantIndex[i]] + intSeedWeight_plant[i] +
			...
			slopeLbeeSeedWeight*exp(visitMu_lbee[plantIndex[i]]); //Claim
...
model {
	...
	// Priors
	slopeLbeeSeedWeight ~ normal(0,1); //Claim
