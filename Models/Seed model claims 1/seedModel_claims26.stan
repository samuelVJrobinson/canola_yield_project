...
parameters {
	//Claim 26: podSuccess ~ seed count
	real slopeSeedCountFlwSurv; //Effect of predicted seed count
...
transformed parameters {			
	...
	for(i in 1:Nplot){
		...
		flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[i]] + intFlwSurv_plot[i] + 
			...
			slopeSeedCountFlwSurv*(intSeedCount + intSeedCount_field[plotIndex[i]] + intSeedCount_plot[i]+slopePolSeedCount*(exp(pollenMu_plot[i])/1000)); //Claim
	}
...
model {
	...
	// Priors
	//Claim
	slopeSeedCountFlwSurv ~ normal(0,1);	
