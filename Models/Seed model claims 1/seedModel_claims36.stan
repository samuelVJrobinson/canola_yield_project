...
parameters { 
	//Claim: FlwCount ~ (log) distance to lbee shelter
	real slopeMinDistFlwCount; 
...
transformed parameters {			
	...
	for(i in 1:Nplot){			
		...
		flwCountPlot[i] = intFlwCount + intFlwCount_field[plotIndex[i]]+ //+ intFlwCount_plot[i];
			slopeMinDistFlwCount*logLbeeDist_all[i]; //Distance to lbee shelter
	}	
...
model {	
	...
	// Priors	
	//Claim
	slopeMinDistFlwCount ~ normal(0,1); 
