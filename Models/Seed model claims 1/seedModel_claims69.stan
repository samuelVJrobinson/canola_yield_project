...
transformed data {
	...
	vector[Nplant_obs] logFlwCount_obs; //(log) number of flowers per plant
	...
	for(i in 1:Nplant_obs) //Log-transform flowers per plant
		logFlwCount_obs[i]=log(flwCount[i]);
...
parameters { 
	//Claim: seed weight ~ flower count per plant
	real slopeFlwCountSeedWeight; 	
	...
	//(log) Flower count
	vector[Nplant_miss] logFlwCount_miss; //Vector for imputing missing values
...
transformed parameters {			
	...
	//(log) Flower count per plant
	logFlwCount[obsPlant_ind]=logFlwCount_obs; //Observed flowers per plant
	logFlwCount[missPlant_ind]=logFlwCount_miss; //Imputed flowers per plant
	...
	for(i in 1:Nplant){			
		...
		seedWeightPlantMu[i] = intSeedWeight + intSeedWeight_field[plotIndex[plantIndex[i]]] + intSeedWeight_plot[plantIndex[i]] + intSeedWeight_plant[i] +
			...
			slopeFlwCountSeedWeight*logFlwCount[plantIndex[i]]; //Flower count per plant
	}
...
model {	
	...
	// Priors	
	//Claim
	slopeFlwCountSeedWeight ~ normal(0,0.1); 
