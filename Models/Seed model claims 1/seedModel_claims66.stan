...
parameters { 
	//Claim: seed weight ~ distance from edge
	real slopeDistSeedWeight; 	
...
transformed parameters {			
	...
	for(i in 1:Nplant){			
		...
		seedWeightPlantMu[i] = intSeedWeight + intSeedWeight_field[plotIndex[plantIndex[i]]] + intSeedWeight_plot[plantIndex[i]] + intSeedWeight_plant[i] +
			...
			slopeDistSeedWeight*logHbeeDist_all[plantIndex[i]]; //Distance from edge
	}
...
model {	
	...
	// Priors	
	//Claim
	slopeDistSeedWeight ~ normal(0,1); 
