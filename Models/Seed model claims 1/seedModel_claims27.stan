...
parameters {
	//Claim 27: podSuccess ~ seed size
	real slopeSeedSizeFlwSurv; //Effect of predicted seed size (per seed)
...
transformed parameters {			
	...
	for(i in 1:Nplot){
		...
		flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[i]] + intFlwSurv_plot[i] + 
			...
			slopeSeedSizeFlwSurv*(intSeedWeight + intSeedWeight_field[plotIndex[i]] + intSeedWeight_plot[i] + slopePolSeedWeight*(exp(pollenMu_plot[i])/1000)); //Claim
	}
...
model {
	...
	// Priors
	//Claim
	slopeSeedSizeFlwSurv ~ normal(0,1);	
