...
parameters { 
	//Claim: FlDens ~ leafcutter stocking
	real slopeLbeeStockingFlDens; 
...
transformed parameters {			
	...
	for(i in 1:Nplot_all){		
		...
		flDensMu[i] = intFlDens	+ intFlDens_field[plotIndex_all[i]] + 
			...
			slopeLbeeStockingFlDens*lbeeStocking_all[i]; //Lbee stocking
	}		
...
model {	
	...
	// Priors	
	//Claim
	slopeLbeeStockingFlDens ~ normal(0,1); 
