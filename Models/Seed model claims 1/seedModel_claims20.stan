...
parameters { 
	//Claim: HbeeVis ~ leafcutter visitation (direct effect)
	real slopeLbeeVisHbeeVis; 
...
transformed parameters {			
	...
	for(i in 1:Nplot_all){		
		...
		visitMu_hbee[i] = intVisitHbee + logTime_all[i] + //intVisitHbee_field[plotIndex_all[i]] + //Intercepts + time offset
			...
			slopeLbeeVisHbeeVis*logLbeeVis_all[i]; //(log) lbee visitation effect
	}	
...
model {
	...
	// Priors	
	//Claim
	slopeLbeeVisHbeeVis ~ normal(0,1); 
