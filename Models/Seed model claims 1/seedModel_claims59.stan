...
parameters { 
	//Claim: seed count ~ survival
	real slopeSurvSeedCount; 	
	...
	//(logit) flower survival
	vector[Nplant_miss] propFlwSurv_miss; //Vector for imputing missing values	
...
transformed parameters {			
	...
	//(logit) flower survival
	propFlwSurv[obsPlant_ind]=propFlwSurv_obs; //Observed flowers per plant
	propFlwSurv[missPlant_ind]=propFlwSurv_miss; //Imputed flowers per plant
	...
	for(i in 1:Nplant){			
		...
		seedCountMuPlant[i] = intSeedCount + intSeedCount_field[plotIndex[plantIndex[i]]] + intSeedCount_plant[i] + //intSeedCount_plot[plantIndex[i]] +
			...
			slopeSurvSeedCount*propFlwSurv[plantIndex[i]]; //Flower survival per plant
	}
