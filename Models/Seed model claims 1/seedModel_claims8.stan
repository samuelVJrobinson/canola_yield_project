...
parameters { 
	//Claim: PlSize ~ lbee stocking
	real slopeLbeeStockingPlSize; 
...
transformed parameters {			
	...
	for(i in 1:Nplot_all){		
		...
		plSizePlotMu[i] = intPlSize + //intPlSize_field[plotIndex_all[i]] + //intPlSize_plot[i] + 			
			...
			slopeLbeeStockingPlSize*lbeeStocking_all[i]; //Effect of leafcutter stocking
	}	
...
model {	
	...
	// Priors	
	//Claim
	slopeLbeeStockingPlSize ~ normal(0,1); 
