...
parameters { 
	//Claim: FlDens ~ Distance from lbee shelter
	real slopeMinDistFlDens; 
...
transformed parameters {			
	...
	for(i in 1:Nplot_all){		
		...
		flDensMu[i] = intFlDens	+ intFlDens_field[plotIndex_all[i]] + 
			...
			slopeMinDistFlDens*logLbeeDist_all[i]; //Distance from lbee shelter
	}		
