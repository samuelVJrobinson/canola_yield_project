...
parameters { 
	//Claim: seed count ~ distance from edge
	real slopeDistSeedCount; 	
...
transformed parameters {			
	...
	for(i in 1:Nplant){			
		...
		seedCountMuPlant[i] = intSeedCount + intSeedCount_field[plotIndex[plantIndex[i]]] + intSeedCount_plant[i] + //intSeedCount_plot[plantIndex[i]] +
			...
			slopePolSeedCount*pollenMu_plot[plantIndex[i]] + //Pollen deposition (plot level average)
			slopeDistSeedCount*logHbeeDist_all[plantIndex[i]]; //Distance from edge
	}
