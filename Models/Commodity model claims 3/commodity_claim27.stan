...
parameters {
  // Plant density	
	...
	real claim27_seedWeightFlwCount; //Claim
	real slopeHbeeVisFlwCount; //Other claim
	real slopePolFlwCount; //Other claim
	real slopeSeedCountFlwCount; //Other claim
...
transformed parameters {		
  //Expected values
    ...
    flwCountPlot[i] = intFlwCount + //Intercept
      slopePolFlwCount*pollenPlot[i] + //Other claim
      slopeHbeeVisFlwCount*logHbeeVis[i] + //Other claim
    ...
    flwCountMu[i] = flwCountPlot[plantIndex[i]] + //Plot level flower count
      claim27_seedWeightFlwCount*seedMass[i] + //Claim
      slopeSeedCountFlwCount*seedCount[i] + //Other claim
...
model {	
	...
	//Flower count (per plant)
	claim27_seedWeightFlwCount ~ normal(0,5); //Claim
	slopeHbeeVisFlwCount ~ normal(0,5); //Other claim
	slopePolFlwCount ~ normal(0,5); //Other claim
	slopeSeedCountFlwCount ~ normal(0,5); //Other claim
