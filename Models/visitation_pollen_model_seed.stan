...
parameters {
	...
	// real slope2016Hbee; //Effect of 2016 		
	...
	// real slope2016Lbee; //Effect of 2016 
	...
	// real slopePlsizeLbee; //Slope of plant size - once properly specified, FlDens is the better path
	...
	// // real slopeLbeeDistSeedCount; //Slope of leafcutter distance on seed count - correlated 	
...
transformed parameters {			
	...
	for(i in 1:Nplot_all){		
		...
		visitMu_hbee[i] = intVisitHbee + logTime_all[i] + //intVisitHbee_field[plotIndex_all[i]] + //Intercepts + time offset
			...
			slopeLbeeVisHbee*lbeeVis_all[i] + //Direct effect of (log) leafcutter visitation						
	...
	// for(i in 1:Nplot){
		...
		// flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[i]] + intFlwSurv_plot[i] + 			
			...
			// slopePolSurv*pollenMu_plot[i]; //Pollen deposition
	...
	for(i in 1:Nplant){
		...
		// seedCountMuPlant[i] = intSeedCount + intSeedCount_field[plotIndex[plantIndex[i]]] + intSeedCount_plant[i] + //intSeedCount_plot[plantIndex[i]] +
			// slopePolSeedCount*pollenMu_plot[plantIndex[i]] + //Pollen counts
		...
		// seedWeightPlantMu[i] = intSeedWeight + intSeedWeight_field[plotIndex[plantIndex[i]]] + intSeedWeight_plot[plantIndex[i]] + intSeedWeight_plant[i] +
			// slopePolSeedWeight*pollenMu_plot[plantIndex[i]]+ slopePlSizeSeedWeight*plantSize[i];
...
model {
	...
	for(i in 1:Nplot_all){ //Zero-inflated negbin for hbee visitation frequency
		if(hbeeVis_all[i]==0){
			target += log_sum_exp(bernoulli_lpmf(1|pow(zeroVisHbeeTheta,totalTime_all[i])),
				bernoulli_lpmf(0|pow(zeroVisHbeeTheta,totalTime_all[i])) + neg_binomial_2_log_lpmf(0|visitMu_hbee[i],visitHbeePhi));
		} else {
			target += bernoulli_lpmf(0|pow(zeroVisHbeeTheta,totalTime_all[i]))+neg_binomial_2_log_lpmf(hbeeVis_all[i]|visitMu_hbee[i],visitHbeePhi);					
		}
	}		
	...
	// // slopeLbeeDistSeedCount ~ normal(0,0.5); //Slope of leafcutter distance on seed count	- correlated with other predictors, and not very strong
...
generated quantities{
	...
	for(i in 1:Nplot_all){
		//bee visits
		hbeeVis_resid[i]=hbeeVis_all[i]-(exp(visitMu_hbee[i])*(1-zeroVisHbeeTheta)); //Residual for actual value
		if(bernoulli_rng(zeroVisHbeeTheta)==1) //If zeroVisHbeeTheta generates an extra zero
			...
			predHbeeVis_all[i] = neg_binomial_2_log_rng(visitMu_hbee[i]-logTime_all[i],visitHbeePhi); //Predicted value drawn from neg.bin		
