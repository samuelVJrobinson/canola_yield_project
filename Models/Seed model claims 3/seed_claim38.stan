...
parameters {
	...
	// Weight per seed
	real claim38_slopeFlwCountSeedWeight; //Claim
	real slopeFlwSurvSeedWeight; //Other claim
	...
	for(i in 1:Nplant){	
	  ...
	  int plotI = plotIndex_F[plantIndex[i]];
		...
		seedWeightMu[i] = seedWeightPlot[plotI] +
  		...
  		claim38_slopeFlwCountSeedWeight*logFlwCount[i] + //Claim
  		slopeFlwSurvSeedWeight*logitFlwSurv[i]; //Other claim
	}
...
model {
	...
	// Weight per seed
	claim38_slopeFlwCountSeedWeight ~ normal(0,5); //Claim
	slopeFlwSurvSeedWeight ~ normal(0,5); //Other claim
