...
parameters {
  ...
  real<lower=2.4,upper=4.5> plDens_miss; //Imputed plant density for my fields (only 1)
	...
	// lbee Visitation
	real claim13_slopePlSizeLbeeVis; //Claim
	real slopePlDensLbeeVis; //Other claim
	  ...
	  // Expected value for lbee visits 
		visitMu_lbee[i] = intLbeeVis + 
		  ...
		  intLbeeVis_field[plotIndex_all[plotI]] + //Intercepts
			...
			claim13_slopePlSizeLbeeVis*(plSizePlotMu[i] - intPlSize) + //Claim
			slopePlDensLbeeVis*plDens[i]; //Other claim
	}	
...
model {
	...
	claim13_slopePlSizeLbeeVis ~ normal(0,5); //Claim
	slopePlDensLbeeVis ~ normal(0,5); //Other claim
