...
parameters {
	//Claim 28: podSuccess ~ stocking
	real slopeStockingFlwSurv; //Direct effect of half-stocking
...
transformed parameters {			
	...
	for(i in 1:Nplot){
		...
		flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[i]] + intFlwSurv_plot[i] + 
			...
			slopeStockingFlwSurv*lbeeStocking_all[i]; //Claim
	}
...
model {
	...
	// Priors
	//Claim
	slopeStockingFlwSurv ~ normal(0,1);	
