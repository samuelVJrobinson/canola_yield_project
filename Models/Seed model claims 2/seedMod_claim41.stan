...
parameters {
	//Claims: flwCount ~ hbee vis
	real slopeHbeeVisFlwCount;	
	real slopeFlDensFlwCount; //Conditioning set - flower density
	real slopeHbeeDistFlwCount; //hbee dist
	real slopeLbeeDistFlwCount; //lbee dist
	real slopeLbeeVisFlwCount; //lbee visits
...
transformed parameters {	
	...
	for(i in 1:Nplot_F){ //Parameters for F plots only	
		...
		flwCountPlot[i] = intFlwCount + intFlwCount_field[plotIndex[plotIndex_F2[i]]] + intFlwCount_plot[i] +
			...
			slopeHbeeVisFlwCount*logHbeeVis_all[plotIndex_F2[i]] + // Claim - hbeeVis
			...
			slopeLbeeVisFlwCount*logLbeeVis_all[plotIndex_F2[i]]; //Conditioning set - lbee visits			
	}	
...
model {	
	...
	// Priors
	// Claim
	slopeHbeeVisFlwCount ~ normal(0,5); // Claim - hbeeVis			
	...
	slopeLbeeVisFlwCount ~ normal(0,5); //Conditioning set - lbeeVis
