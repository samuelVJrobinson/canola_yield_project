...
parameters {
	//Claim 14: seedCount ~ shelter stocking
	real slopeStockingSeedCount; //Direct effect of shelter stocking
...
transformed parameters {			
	...
	for(i in 1:Nplant){
		...
		seedCountMuPlant[i] = intSeedCount + intSeedCount_field[plotIndex[plantIndex[i]]] + intSeedCount_plot[plantIndex[i]] + intSeedCount_plant[i] + 
			...
			slopeStockingSeedCount*(lbeeStocking_all[plantIndex[i]]); //Claim		
...
model {
	...
	// Priors	
	//Claim
	slopeStockingSeedCount ~ normal(0,1);
...
generated quantities{
	int predPollenCount[Nflw]; //Generated pollen counts
	real pollenCount_resid=0; //Summed residual of pollen counts
	real predPollenCount_resid=0; //Summed residual of pollen counts
	
	int predSeedCount[Npod]; //Generated seed counts
	real seedCount_resid=0; //Residual of seed counts
	real predSeedCount_resid=0; //Residual of seed counts		
	
	
	for(i in 1:Nflw){	//Predicted pollen counts	
		predPollenCount[i] = neg_binomial_2_log_rng(pollenMu[i],pollenPhi); //Predicted value drawn from neg.bin		
		pollenCount_resid = pollenCount_resid + fabs((exp(pollenMu[i]))-pollenCount[i]); //Residual for actual value
		predPollenCount_resid = predPollenCount_resid + fabs((exp(pollenMu[i]))-predPollenCount[i]); //Residual for predicted value		
	}
	
	for(i in 1:Npod){ //Predicted seed counts
		predSeedCount[i] = neg_binomial_2_log_rng(seedCountMu[i],seedCountPhi); //Predicted value drawn from neg.bin		
		seedCount_resid = seedCount_resid + fabs((exp(seedCountMu[i]))-seedCount[i]); //Residual for actual value
		predSeedCount_resid = predSeedCount_resid + fabs((exp(seedCountMu[i]))-predSeedCount[i]); //Residual for predicted value		
	}	
}
