...
parameters {
	//Claim 18: seedWeight ~ logDist
	real slopeHbeeDistSeedWeight; //Direct effect of distance from edge
...
transformed parameters {			
	...
	for(i in 1:Nplant){
		...
		seedWeightPlantMu[i] = intSeedWeight + intSeedWeight_field[plotIndex[plantIndex[i]]] + intSeedWeight_plot[plantIndex[i]] + intSeedWeight_plant[i] +
			...
			slopeHbeeDistSeedWeight*logHbeeDist_all[plantIndex[i]]; //Claim
...
model {
	...
	// Priors
	slopeHbeeDistSeedWeight ~ normal(0,1); //Claim
