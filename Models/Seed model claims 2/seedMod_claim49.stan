...
parameters {	
	//Claim: seedCount ~ lbee vis
	real slopeLbeeVisSeedCount;
	real slope2016SeedCount; //Conditioning set
	real slopeLbeeDistSeedCount;
	real slopeStockingSeedCount;	
...
transformed parameters {
	...
	for(i in 1:Nplant){	
		...
		seedCountMuPlant[i] = intSeedCount + //Intercept
			...
			slopeLbeeVisSeedCount*logLbeeVis_all[plantIndex[i]] + //Claim 
			slope2016SeedCount*is2016_all[plantIndex[i]] + //Conditioning set - year
			slopeLbeeDistSeedCount*logLbeeDist_all[plantIndex[i]] + //Conditioning set - lbeeDist
			slopeStockingSeedCount*lbeeStocking_all[plantIndex[i]]; //Conditioning set - stocking
	}
...
model {
	...
	// Priors
	// Claims
	slopeLbeeVisSeedCount ~ normal(0,5); 	
	slope2016SeedCount ~ normal(0,5); //Conditioning set - year
	slopeLbeeDistSeedCount ~ normal(0,5); //LbeeDist
	slopeStockingSeedCount ~ normal(0,5); //Stocking
