...
parameters {
	//Claims: seedWeight ~ lbeeVis
	real slopeLbeeVisSeedWeight;
	real slopeFlDensSeedWeight;	//Conditioning set - flDens
	real slopeHbeeDistSeedWeight; //Conditioning set - hbeeDist
	real slopeCentSeedWeight; //Conditioning set - bay centre	
...
transformed parameters {
	...
	for(i in 1:Nplant){	
		...
		seedWeightPlantMu[i] = intSeedWeight + 
			...
			slopeLbeeVisSeedWeight*logLbeeVis_all[plantIndex[i]] + //Claim			
			slopeFlDensSeedWeight*flDens[plantIndex[i]] + //Conditioning set - flDens
			slopeHbeeDistSeedWeight*logHbeeDist_all[plantIndex[i]] + //Conditioning set - hbeeDist
			slopeCentSeedWeight*isCent_all[plantIndex[i]]; //Conditioning set - bay centre
	}
...
model {	
	...
	// Priors
	// Claims
	slopeLbeeVisSeedWeight ~ normal(0,5);
	slopeFlDensSeedWeight ~ normal(0,5); //Conditioning set - flDens
	slopeHbeeDistSeedWeight ~ normal(0,5); //hbeeDist
	slopeCentSeedWeight ~ normal(0,5); //bay centre				
