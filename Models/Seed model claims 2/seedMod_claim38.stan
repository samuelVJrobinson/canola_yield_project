...
parameters {
	//Claims: flwCount ~ plant density
	real slopePlDensFlwCount;
	real slopeHbeeDistFlwCount; //Conditioning set
...
transformed parameters {
	...
	for(i in 1:Nplot_F){ //Parameters for F plots only
		...
		flwCountPlot[i] = intFlwCount + intFlwCount_field[plotIndex[plotIndex_F2[i]]] + intFlwCount_plot[i] +
			...
			slopePlDensFlwCount*plDens[plotIndex_F2[i]] + //Claim			
			slopeHbeeDistFlwCount*logHbeeDist_all[plotIndex_F2[i]]; //Conditioning set
	}	
...
model {	
	...
	// Priors
	// Claim
	slopePlDensFlwCount ~ normal(0,5);
	slopeHbeeDistFlwCount ~ normal(0,5); //Conditioning set
