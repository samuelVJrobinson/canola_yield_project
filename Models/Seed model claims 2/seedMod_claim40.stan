...
parameters {
	//Claims: flwCount ~ lbee vis
	real slopeLbeeVisFlwCount;	
	real slopeHbeeDistFlwCount; //Conditioning set - hbee dist
	real slope2016FlwCount; //year
	real slopeFlDensFlwCount; //flower density
	real slopeLbeeDistFlwCount; //lbee dist
	real slopeStockingFlwCount; //stocking
...
transformed parameters {	
	...
	for(i in 1:Nplot_F){ //Parameters for F plots only
		...
		flwCountPlot[i] = intFlwCount + intFlwCount_field[plotIndex[plotIndex_F2[i]]] + intFlwCount_plot[i] +
			...
			slopeLbeeVisFlwCount*logLbeeVis_all[plotIndex_F2[i]] + // Claim - lbeeVis			
			slopeFlDensFlwCount*flDens[plotIndex_F2[i]] + //Conditioning set - flDens
			slopeHbeeDistFlwCount*logHbeeDist_all[plotIndex_F2[i]] + //Conditioning set - hbeeDist
			slope2016FlwCount*is2016_all[plotIndex_F2[i]] + //Conditioning set - year			
			slopeLbeeDistFlwCount*logLbeeDist_all[plotIndex_F2[i]] + //Conditioning set - lbee dist
			slopeStockingFlwCount*lbeeStocking_all[plotIndex_F2[i]]; //Conditioning set - stocking			
	}	
...
model {	
	...
	// Priors
	// Claim
	slopeLbeeVisFlwCount ~ normal(0,5); // Claim - lbeeVis			
	slopeFlDensFlwCount ~ normal(0,5); //Conditioning set - flDens
	slopeHbeeDistFlwCount ~ normal(0,5); //Conditioning set - hbeeDist
	slope2016FlwCount ~ normal(0,5); //Conditioning set - year			
	slopeLbeeDistFlwCount ~ normal(0,5); //Conditioning set - lbee dist
	slopeStockingFlwCount ~ normal(0,5); //Conditioning set - stocking
