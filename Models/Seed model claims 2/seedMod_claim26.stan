...
parameters {
	...
	//Claim: pollen ~ plSize
	real slopePlSizePol;
	real slopePlDensPol; //Conditioning set 
...
transformed parameters {
	...
	for(i in 1:Nplot_F){ //Parameters for F plots only
		...
		pollenMu_plot[i] = intPol_field[plotIndex[plotIndex_F2[i]]] + intPol_plot[i] + //Intercept + field/plot level random effects 
			...
			slopePlDensPol*plDens[plotIndex_F2[i]] + //Conditioning set - plDens
			slopePlSizePol*plSizePlotMu[plotIndex_F2[i]]; //Claim
	}	
...
model {	
	...
	// Priors
	// Claim
	slopePlSizePol ~ normal(0,5); 
	slopePlDensPol ~ normal(0,5); //Conditioning set
