...
parameters {
	//Claims: flwCount ~ flDens
	real slopeFlDensFlwCount;
...
transformed parameters {
	...
	for(i in 1:Nplot_F){ //Parameters for F plots only
		...
		flwCountPlot[i] = intFlwCount + intFlwCount_field[plotIndex[plotIndex_F2[i]]] + intFlwCount_plot[i] +
			...
			slopeFlDensFlwCount*flDens[plotIndex_F2[i]] + //Claim			
...
model {	
	...
	// Priors
	// Claim
	slopeFlDensFlwCount ~ normal(0,5);
	slopeHbeeDistFlwCount ~ normal(0,5); //Conditioning set - hbeeDist
	slope2016FlwCount ~ normal(0,5); //year	
