...
parameters {
	...
	real slope2016Surv; // Conditioning set - year
	real slopeStockingSurv; // stocking
...
transformed parameters {
	...
	for(i in 1:Nplot_F){ //Parameters for F plots only
		...
		flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[plotIndex_F2[i]]] + intFlwSurv_plot[i] + 			
			...
			slopeLbeeVisSurv*logLbeeVis_all[plotIndex_F2[i]] + //Claim
			slope2016Surv*is2016_all[plotIndex_F2[i]] + //Conditioning set - year
			slopeStockingSurv*lbeeStocking_all[plotIndex_F2[i]]; //Conditioning set - stocking			
	}	
...
model {	
	...
	// Priors
	//Claims
	slopeLbeeVisSurv ~ normal(0,5);
	slope2016Surv ~ normal(0,5); //Conditioning set
	slopeStockingSurv ~ normal(0,5);
