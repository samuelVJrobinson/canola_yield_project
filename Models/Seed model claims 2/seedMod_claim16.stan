...
parameters {
	...
	real slopePlDensLbeeVis;  //Plant density - conditioning set
...
transformed parameters {
	...
	for(i in 1:Nplot_all){	//Parameters for all fields, all plots
		...
		visitMu_lbee[i] = intVisitLbee + intVisitLbee_field[plotIndex_all[i]] + logTime_all[i] + //intercepts + time offset
			...
			slopePlDensLbeeVis*plDens[i] + //Plant density
			slopePlSizeLbeeVis*plSizePlotMu[i]; //Claim
	}	
...
model {
	...
	// Priors
	// Claim
	slopePlSizeLbeeVis ~ normal(0,5);
	slopePlDensLbeeVis ~ normal(0,5); //Conditioning set
