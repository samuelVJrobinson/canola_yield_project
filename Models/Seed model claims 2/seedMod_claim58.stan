...
transformed parameters {
	...
	for(i in 1:Nplant){	
		...
		seedWeightPlantMu[i] = intSeedWeight + 
			...
			slopeFlwCountSeedWeight*logFlwCount[i] + //Claim
			slopeSurvSeedWeight*logitFlwSurv[i] + //Conditioning set - Flower survival
...
model {	
	...
	// Priors
	// Claims
	slopeFlwCountSeedWeight ~ normal(0,5);	
	slopeCentSeedWeight ~ normal(0,5); //bay centre				
	slopeSurvSeedWeight ~ normal(0,5); //survival
