...
parameters {	
	//Claim: seedCount ~ hbee vis
	real slopeHbeeVisSeedCount;
	real slopeLbeeDistSeedCount; //Conditioning set - lbee dist
	real slopeLbeeVisSeedCount;	 //Lbee vis	
...
transformed parameters {
	...
	for(i in 1:Nplant){	
		...
		seedCountMuPlant[i] = intSeedCount + //Intercept
			...
			slopeHbeeVisSeedCount*logHbeeVis_all[plantIndex[i]] + //Claim 
			slopeLbeeDistSeedCount*logLbeeDist_all[plantIndex[i]] + //Conditioning set - lbeeDist
			slopeLbeeVisSeedCount*logLbeeVis_all[plantIndex[i]]; //Conditioning set - lbeeVis
	}
...
model {
	...
	// Priors
	// Claims
	slopeHbeeVisSeedCount ~ normal(0,5); 	
	slopeLbeeVisSeedCount ~ normal(0,5); //Conditioning set - lbeeVis
