data {
	...
	vector[Npod] seedMass; //Weight per seed (mg)
...
parameters {
	//Claim 20: seedWeight ~ stocking
	real slopeStockingSeedWeight; //Direct effect of half-stocking leafcutter shelters
...
transformed parameters {			
	...
	for(i in 1:Nplant){
		...
		seedWeightPlantMu[i] = intSeedWeight + intSeedWeight_field[plotIndex[plantIndex[i]]] + intSeedWeight_plot[plantIndex[i]] + intSeedWeight_plant[i] +
			...
			slopeStockingSeedWeight*lbeeStocking_all[plantIndex[i]]; //Claim
...
model {
	...
	// Priors
	slopeStockingSeedWeight ~ normal(0,1); //Claim
...
generated quantities{
	real predSeedWeight[Npod]; //Generated seed weight
	real seedWeight_resid=0; //Residual of seed weight
