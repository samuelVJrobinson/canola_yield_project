...
parameters {
	//Claim: pollen ~ flower counts
	real slopeFlDensPollen;
...
transformed parameters {		
	...
	for(i in 1:Nplot){ 		
		// Plot-level pollen deposition = random int field + (global intercept later) ...
		pollenPlot[i] = intPollen_field[plotIndex[i]] + 
			slopeVisitPol*logHbeeVis[i] +  //(log) hbee visits							
			slopeFlDensPollen*flDens[i]; //flower density
	}
...
model {		
	...
	//Priors	
	//Claim
	slopeFlDensPollen ~ normal(0,0.1);	
