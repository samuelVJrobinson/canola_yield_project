...
parameters {
	...
	vector[Nplot_flwCountMiss] plSizePlot_miss; //Missing average plant size
}
...
transformed parameters {		
	...
	vector[Nplot] plSizePlot; //Average plant size per plot
	plSizePlot[obsFlwCount_ind] = plSizePlot_obs;
	plSizePlot[missFlwCount_ind] = plSizePlot_miss;
	...
	for(i in 1:Nplot){ 
		...
		pollenPlot[i] = intPollen_field[plotIndex[i]] + //intPollen_plot[i] + 
			...
			slopePlSizePollen*plSizePlot[i]; //Claim
	}
...
model {	
	...
	//Priors
	//Claim:
	slopePlSizePollen ~ normal(0,1);
	plSizePlot_miss ~ normal(0,5); //Prior for imputed data
