...
parameters {
	...
	// real slopeYearVis; //Effect of 2015 - probably negative
	// real slopeGpVis; //Effect of Grand Prairie
	// real slopeYearGpVis; //Year-area interaction
	...
	// real<lower=0> sigmaPolPlot; //SD of plot random intercepts - wasn't converging well
...
transformed parameters {		
	...
	for(i in 1:Nplot){ 
		...
		// visitHbeeMu[i] = intVisit + intVisit_field[plotIndex[i]] + logTime[i] + 
			...
			// slopeHiveVis*numHives[plotIndex[i]] + //Number of hives
			// slopeFlDens*flDens[i]; //Flower density
		
		//Plot-level pollen deposition = intercept + random int field + random int plot + 
		pollenPlot[i] = intPollen + intPollen_field[plotIndex[i]] + //intPollen_plot[i] + 
		...
		flwSurvPlot[i] = intFlwSurv + intFlwSurv_field[plotIndex[i]] + intFlwSurv_plot[i] + 
			...
			slopePolSurv*pollenPlot[i]; //(log) pollen deposition
...
model {
	...
	// bernLL[1]=bernoulli_lpmf(0|zeroVisHbeeTheta); //LL of no extra zero
	// bernLL[2]=bernoulli_lpmf(1|zeroVisHbeeTheta); //LL of extra zero	
	// for(i in 1:Nplot){ //Zero-inflated negbin for visitation frequency
		// if(hbeeVis[i]==0)
			// target += log_sum_exp(bernLL[2],bernLL[1]+neg_binomial_2_log_lpmf(0|visitHbeeMu[i],visitHbeePhi));
		// else
			// target += bernLL[1]+neg_binomial_2_log_lpmf(hbeeVis[i]|visitHbeeMu[i],visitHbeePhi);	
	// }				
	//Likelihood		
	...
	sigmaPlDens_field ~ cauchy(3,10); //Sigma for field
	...
	// slopeHiveVis ~ normal(0,0.5); //Slope of hive effect on visits 
	// slopeYearVis ~ normal(0,1); //2015 effect
	// slopeGpVis ~ normal(0,1); //Effect of Grand Prairie
	// slopeYearGpVis ~ normal(0,1); //Year-area interaction
	...
	// sigmaPolPlot ~ gamma(1.5,5); //Sigma for random plot	- wasn't converging well
	...
	// slopePlSizeFlwCount ~ normal(0.5,1); //Slope of plant size
	...
	// flwCountPhi ~ gamma(1,1); //Dispersion parameter	
	
	//Flower survival - informative priors
	intFlwSurv ~ normal(1.2,1); //Intercept
	slopeVisitSurv ~ normal(0,0.1); //Slope of hbee visits
	slopePolSurv ~ normal(0,0.5); //Slope of pollen deposition
	slopePlSizeSurv ~ normal(0,0.05); //Slope of plant size
	...
	// intSeedCount ~ normal(3.1,1); //Intercept
	...
	// seedCountPhi ~ gamma(12,1); //Dispersion parameter
	...
	// slopePlSizeWeight ~ normal(0,0.1); //Slope of plant size
...
//Plot-level quantities
	...
	for(i in 1:Nplot){
		...
		// //bee visits
		// // hbeeVis_resid[i]=logHbeeVis[i]-(visitHbeeMu[i]+log(1-zeroVisHbeeTheta)); //log-scale residual for actual value
		// hbeeVis_resid[i]=hbeeVis[i]-(exp(visitHbeeMu[i])*(1-zeroVisHbeeTheta)); //Residual for actual value
		// if(bernoulli_rng(zeroVisHbeeTheta)==1) //If zeroVisHbeeTheta generates an extra zero
			// predHbeeVis[i] = 0; //Predicted value is automatically zero
		// else //Otherwise
			// predHbeeVis[i] = neg_binomial_2_log_rng(visitHbeeMu[i],visitHbeePhi); //Predicted value drawn from neg.bin		
		// // predHbeeVis_resid[i]=log(predHbeeVis[i]+0.5)-(visitHbeeMu[i]+log(1-zeroVisHbeeTheta)); //Residual for predicted value			
		// predHbeeVis_resid[i]=predHbeeVis[i]-(exp(visitHbeeMu[i])*(1-zeroVisHbeeTheta)); //log-scale residual for predicted value					
	}		
