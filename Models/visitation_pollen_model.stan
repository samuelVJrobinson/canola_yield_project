...
transformed parameters {		
	...
	for(i in 1:Nplant){
		...
		// Predicted yield = intercept + log(calculated yield)	
		logYieldMu[i] = intYield+log(calcYield[i])*slopeYield;	
	}
}
...
model {	
	...
	//Yield per plant
	intYield ~ normal(0,3); //Intercept
	slopeYield ~ normal(1,3); //Slope of calculated yield
