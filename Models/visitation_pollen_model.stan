...
transformed data {
	...
	vector[Npod] logSeedCount; //Log seed count	(per pod)
...
parameters {
	...
	// Interactions
	// real slope2015IrrigSeedWeight; //Year:irrigation - p=0.21
...
model {	
	...
	phiFlwCount_field ~ gamma(2,20); //SD of field-level random effect		
	intFlwCount_field ~ normal(0,phiFlwCount_field); //Field-level random effect		
	intPhiFlwCount ~ normal(3.7,0.5); //Terms for variance
	slopePlSizePhiFlwCount ~ normal(0.7,0.2);
	sigmaPhiFlwCount_field ~ gamma(2,3); //Sigma for field level sigma
