...
parameters {
	...
	// // real slopeYearGpVis; //Effect of year-GP interaction - appears to be 0
	...
	// real<lower=0> sigmaPolPlot; //SD of plot random intercepts - Rhat 1.05, small n_eff, strongly correlated with lp__
	...
	// vector[Nplant] intSeedCount_plant; //plant-level random intercepts - OK
	// vector[Nplot] intSeedCount_plot; //plot-level random intercepts	- not converging well, Rhat 1.1, small n_eff. 
	// vector[Nfield] intSeedCount_field; //field-level random intercepts - OK
...
model {
	...
	// podCount ~ binomial_logit(flwCount,flwSurv); //Flower survival (surviving pods)	
	...
	// slopeYearVis ~ normal(1,1); //2015 effect
	...
	// intFlwSurv ~ normal(2,1); //Intercept
	...
	// sigmaFlwSurv_plot ~ gamma(3,10); //SD of plot random effect	
