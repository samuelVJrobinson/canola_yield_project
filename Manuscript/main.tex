...
After the models were properly fit and validated, we simulated seed size and yield at varying distances from pollinators, using the coefficients for each path in order to show the integrated effect of pollination on yield (while holding plant density constant).
...
This revealed that not only total seed size and overall yield lower at the bay centre, but that the effect of HB distance is effectively \emph{nil} at the bay centre (yield isolines are essentially horizontal in bay centre, and slightly tilted at the bay edge, Figure \ref{fig:allYield_bayCent} second column).
Interestingly, the simulations also showed evidence of nonlinear behaviour occurring in the centre of seed field bays, where a local minima in yield appears at approximately 25 m away from HB hives and 45 m away from LCB shelters (Figure \ref{fig:allYield_bayCent}) bottom-right panel), likely because of an interaction between seeds per pod, pod count, and seed weight. 


\begin{figure}
    \centering
    \includegraphics[width=\textwidth,keepaspectratio=true]{../Figures/allYield.png}
    \caption{Effect of honey bee distance and leafcutter distance on seed size (mg/seed, panels a \& c) and total yield (t/ha, panels b \& d), using coefficients from paths for simulation, while holding plant density constant at its mean value. Shaded areas shown in a \& b are 95\% quantiles for the simulations. Lines on panels c \& d are isolines of constant seed size (c) or yield (d).}
    \label{fig:allYield}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth,keepaspectratio=true]{../Figures/allYield_bayCent.png}
    \caption{Simulation results from seed field models, showing seed size and yield (first and second rows, respectively) for plots at the edge and centre of female bays (first and second column), at varying distances away from HB hives and LCB shelters.}
    \label{fig:allYield_bayCent}
\end{figure}


\section*{Discussion}

This study examined how visitation contributes to pollen deposition and seed yield in two canola crops, and showed a strong link between visitation and yield in seed canola, but not in commodity canola.
...
HBs were never the aggressor in these interactions, so we treated high leafcutter visitation as the cause of lower HB visitation in our SEM (Figure \ref{fig:allSEM}, arrow from leafcutter visit rate to HB visit rate).

HB visitation and foraging behaviour varied strongly between the male and female bays of seed fields, as well as within the female bays.
Visitation rates of HB foragers were almost twice as high in the centre of the female bay, and conversely, LCB visitation was almost twice as low.
This could be due to LCBs avoiding competition for nectar with HBs, but more likely reflects a stronger need for pollen among LCBs than HBs \citep{cane2011}.
Only 1.4\% of the HBs in the female bay were pollen foragers (Table \ref{tab:sideWorking}), meaning that foragers who have (recently) come into contact with pollen rare.
...
Therefore, HBs pollen foragers in seed canola fields appear to avoid flowers in the female bay, and commonly engage in side-working behaviour in both commodity and seed canola fields, limiting their opportunities for pollen transfer (see below).
...
However, \citet{waytesMsc} used male-sterile flowers and found that \textit{A. mellifera} deposit far less outcrossed pollen ($\sim$ 2 grains per visit), suggesting that much of the pollen deposited by HBs on canola stigmas is self-pollen. 
Because the overall pollen deposition rates found in this study were so high (mean: 293, SD: 385), HB visitation seems to have made little difference in the amount of self-pollination.
...
These results suggest that stigmas are largely saturated with self-pollen, swamping any extra pollen deposition by HBs.
...
%REVIEW THIS
HB behaviour in seed fields made pollen transfer between male and female plants unlikely, and this was also reflected in the measurements of stigma deposition. 
Deposition was not affected by HB visitation, but was strongly increased by LCB visitation.
LCBs tend to switch between male and female flowers more frequently than HBs do \citep{waytesMsc}, transport more viable pollen between flowers \citep{parker2015}, and tend not to side-work canola flowers (\citealp{soroka2001}, personal observation), which may explain their increased pollination efficacy.
...
During foraging bouts, LCBs may travel further between flowers than HBs do; \citet{brunet2019} found that LCBs foraging in alfalfa (\emph{Medicago sativa} L.) travelled an average of 21 cm between flowers, while HBs only travelled 16 cm.
However, pollen carryover to the middle of the bay female is likely reduced, as leafcutters likely visit the edge of the female bay before venturing into the centre \citep{thomson1986, pinnisch1990}.
...
Pod set in commodity canola was positively affected by plant size, but not by pollen deposition or HB visitation.
Both pod production and seeds per pod can increase with extra pollination \citep{jauker2008,sabbahi2005,sabbahi2006,duran2010}, so this lack of effect may be due to a high overall level of pollen deposition (Figure \ref{fig:allPollen}), even in fields without HBs.
There was no effect of HB visitation on pod set, seeds per pod, or seed size, indicating that visitation did not seem to cause either \textit{a)} a greater quantity of pollen (see above) or \textit{b)} higher quality (outcrossed) pollen.
