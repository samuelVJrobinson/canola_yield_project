...
Seed fields were also stocked with shelters (1.3 - 2.6 shelters/ha) containing alfalfa leafcutter bee cocoons (\textit{Megachile rotundata} F., hereafter LCB) at a rate of 50,000-100,000 coocoons/ha (Figure \ref{fig:seedfieldPhoto}, \ref{fig:seedfieldPlots}). 
...
The path analysis revealed that plant size and pollen deposition were the main drivers of seed production in seed canola, while plant size alone was the main driver in commodity canola (see Figure \ref{fig:allSEM} for paths and effect sizes). 
...
Finally, there was an indirect effect (not related to visitation) of bay centre, distance from edge, and distance from LCB shelter on pollen count and pod set in seed fields, pointing to possible differences in pollen transfer even at the same rate of visitation. 
...
Plant size had by far the strongest positive effect on the number of pods produced, and there was evidence of pod set suppressing flower production (see plant paths in Figure \ref{fig:allSEM}), but pollen only directly affected pod and seed production in seed canola.
...
Pollen deposition did not alter seed production in commodity canola (Figure , nor was there a direct effect of honey bee visitation or distance from field edge (note log-scaled x-axis on Figure \ref{fig:allSeeds}).
