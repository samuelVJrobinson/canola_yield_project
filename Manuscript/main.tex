...
Random intercepts were used to model variance at the field level (and plot level, for data collected within plots); however, plot-level variance terms 
% Plot-level RE omitted from: commodity pollen, commodity flwSurv, commodity seedCount, seed plDens 
...
Plant and pollen samples were not collected from male bays, and there were no large differences in visitation rates for honey bees or LCBs (p = 0.32, 0.72, respectively) once other terms were included as covariates, so bay identity was excluded from further analysis.
...
We assessed the underlying probability distribution functions of each model using posterior predictive checks \citep{gelman2013}, and found that the probability distributions were adequately specified (p$\sim$0.5).
...
% Pollen deposition increased pod set (Z=3.1) and seeds per pod (Z=2.3) in seed canola, but was not present in commodity canola, showing that yield in seed canola, but not commodity canola, is strongly dependent on pollination.
...
% Finally, in both crop types, the paths leading from plant size were the dominant signal influencing pod set (commodity Z=5.8; seed Z=19.9), but the path from pollen deposition to pod set was only present in seed canola, showing that plant resources matter strongly for both crop types, but that increased pollen deposition only benefits seed canola.
% 
...
In commodity fields, honey bee visitation decreased from 8.5 visits/hr at the edge of the field to 3.1 visits/hr at 100 m into the field (p$=$0.0002, Figure \ref{fig:hbeeDist_commodity}, given a stocking rate of 20 hives).
...
Higher stocking rates increased honey bee visitation at the edge of the field from 14.5 visits/hr with an apiary of 20 hives to 22.7 visits/hr with an apiary of 40 hives (Z=5.3, p$<$0.0001), while unstocked field edges had an average of 1.8 visits/hr.
Honey bee visitation was much higher overall in seed canola fields, but still decreased from 126 visits/hr at edge of field to 78 visits/hr at 100 m from the apiary (Z=-2.5, p$=$0.01). 
...
Pollen deposition also decreased from 23 grains/stigma at the edge of the female bay to 13 at the centre of the bay independent of visitation (Z=-4.5, p$=$0.0001), suggesting that pollen may be lost from bees crossing over from male bays \citep{pinnisch1990}.%, who likely visit flowers at the edge of the bay first. 
