...
Pollination, however, is largely treated as a single process, even though there are many links in the chain of events that result in the production of a fruit.
Visitation of flowers by animals (typically bees or other insects) can increase pollen deposition on the stigma of the flower, followed by fertilization of the ovules, and maturation of the ovaries into mature fruit \citep{erbar2003, goldberg1994}.
Each step has many other underlying processes (\textit{e.g.} stigma receptivity, pollen tube growth, ovule abortion, fruit maturation, reviewed in \citealp{real1983, giovannoni2001, erbar2003, takayama2005}, and others), but even at this level of approximation, most studies of agricultural pollination fail to account for important underlying processes.
...
\subsection{Insect visitation}

Pollinator visitation is not spatially uniform within fields, as visitation rates typically decline with distance away from their nest site.
...
Pollination of hybrid seed crops presents a conundrum: it requires movement of pollen between separate lines of plants, meaning that fidelity can reduce the pollen transfer in these systems \citep{waytesMsc, gaffney2019}.
Competition between different species of foraging insects can occur if they deplete floral resources, but foragers can spatially or temporally separate their foraging to reduce competition (\citealp{schaffer1979,thomson1987}, but see \citealp{steffan2000}). This can motivate visitors to reduce competition by increasing their foraging distance, switching between floral morphs more often, or visiting less frequently \citep{heinrich1979, greenleaf2006}.
...
Many flowering plant species are capable of self-pollination, but typically produce more fruit or seeds from outcrossed pollen \citep{knight2005} transported from conspecific plants by wind or an animal vector.
Not all animal visitors are equal from a flowering plant's perspective: large-bodied insects can deposit more pollen per visit on the stigma of the flower \citep{sahli2007}.
Similarly, not all pollinators behave the same: flower-visiting insects may steal nectar, groom pollen between visits, or travel mainly between flowers on the same plant \citep{thomson1986, herrera1987}.
...
Again, this is seldom considered within agricultural systems, but some do examine optimal rates of stocking.

\subsection{Plant resources}
...
We found mixed support across both crop types: seed size and number in commodity canola was mainly influenced by plant size, while seed size and number in seed canola was influenced by both plant size and pollen deposition.
...
Pollen deposition had a smaller effect on seeds per pod than plant size in seed canola, again suggesting that plant resources limit seed production more strongly than pollen deposition, even in pollen-limited crops \citep{marini2015}.
In contrast, we found that seed size increased with plant size in seed canola alone, and was not influenced by pollen or visitation in either crop.
...
Our model serves as an approximation of a dynamic process, but could be built upon by using dynamic linear models \citep{iwasa2000,nord2011} to examine how plants dynamically respond to changes in pollination.
